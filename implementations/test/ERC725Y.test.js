...
contract("ERC725Y (from EOA)", (accounts) => {
  ...
  context("write Solidity variable types as raw bytes", async () => {
    ...
    it("should write a `string` to storage", async () => {
      let key = KEY_FOR_ADDRESS;
      let value = web3.eth.abi.encodeParameter(
        "string",
    ...
    context("fetching Solidity value types (stored as raw bytes)", async () => {
      const runs = [
        ...
        },
        {
          ...
          value: web3.eth.abi.encodeParameter("bytes4[]", ["0xaaaaaaaa"]),
        },
        {
          name: "bytes4[] (x5 values)",
          key: web3.utils.soliditySha3("bytes4[] (x5 values)"),
          value: web3.eth.abi.encodeParameter("bytes4[]", [
            "0xaaaaaaaa",
            "0xbbbbbbbb",
            "0xcccccccc",
            "0xdddddddd",
        ...
        },
        {
          ...
          value: web3.eth.abi.encodeParameter("bytes32[]", [
            "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
          ]),
        },
        {
...
contract("ERC72YXInit", (accounts) => {
  context("after deploying the base contract", async () => {
    ...
    it("Supports ERC165", async () => {
      assert.isTrue(await erc725YInit.supportsInterface.call(INTERFACE_ID.ERC165));
    });

    it("Supports ERC725Y", async () => {
      assert.isTrue(await erc725YInit.supportsInterface.call(INTERFACE_ID.ERC725Y));
    });
  });
});
