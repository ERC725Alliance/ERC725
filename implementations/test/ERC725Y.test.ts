...
describe("ERC725Y", () => {
  describe("when using ERC725Y contract with constructor", () => {
    const buildTestContext = async (): Promise<ERC725YTestContext> => {
      ...
      const erc725Y = await new ERC725Y__factory(accounts.owner).deploy(
  ...
  describe("when using ERC725Y contract with proxy", () => {
    const buildTestContext = async (): Promise<ERC725YTestContext> => {
      ...
      const erc725YBase = await new ERC725YInit__factory(
        accounts.owner
      ).deploy();

      const erc725YProxy = await deployProxy(
        erc725YBase.address,
        accounts.owner
      );
    ...
    const initializeProxy = async (context: ERC725YTestContext) => {
      return context.erc725Y["initialize(address)"](
    ...
    describe("when deploying the base implementation contract", () => {
      it("prevent any address from calling the initialize(...) function on the implementation", async () => {
        ...
        await expect(
          erc725YBase["initialize(address)"](randomCaller.address)
